...
# rds_cluster.py
#
# Adapted from existing Ansible module rds_cluster.py
# https://github.com/manicminer/ansible-modules-rds-cluster/blob/master/library/rds_cluster.py
#
...
options:
  ...
  db_cluster_parameter_group:
    description:
      - DB cluster parameter group for the new cluster
    default: null
  state:
    description:
...
def create_cluster(module, client, **params):
    ...
    if params['db_cluster_parameter_group'] is not None:
        api_args['DBClusterParameterGroupName'] = params['db_cluster_parameter_group']
    ...
    if params['storage_encrypted'] is not None:
        api_args['StorageEncrypted'] = params['storage_encrypted']
    if params['kms_key'] is not None:
        api_args['KmsKeyId'] = params['kms_key']
    if params['backup_retention'] is not None:
        api_args['BackupRetentionPeriod'] = params['backup_retention']
    if params['iam_database_authentication'] is not None:
        api_args['EnableIAMDatabaseAuthentication'] = params['iam_database_authentication']
    ...
    try:
        ...
        for opt, val in api_args.iteritems():
            if opt == 'Tags':
                continue
            elif opt == 'AvailabilityZones':
                continue
            elif opt == 'KmsKeyId':
                continue
            elif opt == 'EnableIAMDatabaseAuthentication':
                if cluster['IAMDatabaseAuthenticationEnabled'] != val:
                  modify_args[opt] = val
            elif opt == 'VpcSecurityGroupIds':
                if sorted([g['VpcSecurityGroupId'] for g in cluster['VpcSecurityGroups']]) != sorted(val):
                    modify_args[opt] = val
            elif opt == 'DBClusterParameterGroupName':
                if cluster['DBClusterParameterGroup'] != val:
                  modify_args[opt] = val
...
def main():
    module_args = dict(
        ...
        engine=dict(required=False, choices=['aurora','aurora-mysql'], default='aurora-mysql'),
        engine_version=dict(required=False),
        master_username=dict(required=False),
        master_password=dict(required=False, no_log=True),
        db_cluster_parameter_group=dict(required=False),
        ...
        iam_database_authentication=dict(type='bool', required=False, default=False),
        backup_retention=dict(type='int', required=False),
