...
#
# Adapted from existing Ansible module rds_param_group.py
# (https://github.com/ansible/ansible-modules-core/blob/devel/cloud/amazon/rds_param_group.py)
#
...
description:
     - Creates, modifies, and deletes RDS cluster parameter groups. This module has a dependency on python-boto >= 2.5.
requirements: [ boto3 ]
options:
  state:
    description:
  ...
  name:
    description:
      - Cluster parameter group identifier.
    required: true
  description:
    description:
      - Cluster parameter group description. Only set when a new group is added.
  engine:
    description:
...
EXAMPLES = '''
# Add or change a cluster parameter group, in this case setting auto_increment_increment to 42 * 1024
...
db_cluster_parameter_group_name:
    description: Name of cluster parameter group
    type: string
    returned: when state is present
db_cluster_parameter_group_family:
    description: Cluster parameter group family that this cluster parameter group is compatible with.
    type: string
    returned: when state is present
...
def update_parameters(module, connection):
    ...
    errors = []
    modify_list = []
    parameters_paginator = connection.get_paginator('describe_db_cluster_parameters')
    existing = parameters_paginator.paginate(DBClusterParameterGroupName=groupname).build_full_result()['Parameters']
    ...
    if modify_list:
        ...
        for modify_slice in zip_longest(*[iter(modify_list)] * 20, fillvalue=None):
            ...
            try:
                connection.modify_db_cluster_parameter_group(DBClusterParameterGroupName=groupname, Parameters=non_empty_slice)
...
def update_tags(module, connection, group, tags):
    changed = False
    existing_tags = connection.list_tags_for_resource(ResourceName=group['DBClusterParameterGroupArn'])['TagList']
    to_update, to_delete = compare_aws_tags(boto3_tag_list_to_ansible_dict(existing_tags),
                                            tags, module.params['purge_tags'])
    if to_update:
        try:
            connection.add_tags_to_resource(ResourceName=group['DBClusterParameterGroupArn'],
    ...
    if to_delete:
        try:
            connection.remove_tags_from_resource(ResourceName=group['DBClusterParameterGroupArn'],
                                                 TagKeys=to_delete)
            changed = True
        except botocore.exceptions.ClientError as e:
...
def ensure_present(module, connection):
    ...
    errors = []
    try:
        response = connection.describe_db_cluster_parameter_groups(DBClusterParameterGroupName=groupname)
    except botocore.exceptions.ClientError as e:
        if e.response['Error']['Code'] == 'DBParameterGroupNotFound':
            ...
            module.fail_json(msg="Couldn't access parameter group information: %s" % str(e),
                             ...
                             **camel_dict_to_snake_dict(e.response))
    if not response:
        params = dict(DBClusterParameterGroupName=groupname,
                      DBParameterGroupFamily=module.params['engine'],
                      Description=module.params['description'])
        if tags:
            params['Tags'] = ansible_dict_to_boto3_tag_list(tags)
        try:
            response = connection.create_db_cluster_parameter_group(**params)
            changed = True
        except botocore.exceptions.ClientError as e:
    ...
    try:
        ...
        module.fail_json(msg="Couldn't obtain parameter group information: %s" % str(e),
                         ...
                         **camel_dict_to_snake_dict(e.response))
    try:
        tags = connection.list_tags_for_resource(ResourceName=group['db_cluster_parameter_group_arn'])['TagList']
    except botocore.exceptions.ClientError as e:
...
def ensure_absent(module, connection):
    ...
    try:
        response = connection.describe_db_cluster_parameter_groups(DBClusterParameterGroupName=group)
    except botocore.exceptions.ClientError as e:
        if e.response['Error']['Code'] == 'DBParameterGroupNotFound':
            ...
            module.fail_json(msg="Couldn't access parameter group information: %s" % str(e),
                             ...
                             **camel_dict_to_snake_dict(e.response))
    try:
        response = connection.delete_db_cluster_parameter_group(DBClusterParameterGroupName=group)
